...
def get_users_list_keyboard(users: List[Dict], page: int = 0) -> InlineKeyboardMarkup:
    ...
    for user in page_users:
        ...
        if user.get('is_blocked', False):
            status = "🔒"
        elif user.get('subscribed', False):
            status = "🟢"
        else:
            status = "🔴"
    ...
    keyboard.append([
        InlineKeyboardButton(text="➕ Добавить вручную", callback_data="users_add_manual"),
...
def get_user_info_keyboard(user_id: int, is_blocked: bool = False) -> InlineKeyboardMarkup:
    buttons = [
        ...
        [InlineKeyboardButton(text="🔄 Переключить подписку", callback_data=f"user_toggle_{user_id}")],
    ]
...
def get_mailing_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        ...
        [InlineKeyboardButton(text="🟢 Только подписанным", callback_data="mailing_subscribed")],
        [InlineKeyboardButton(text="🔴 Только отписавшимся", callback_data="mailing_unsubscribed")],
        [InlineKeyboardButton(text="⬅️ Назад", callback_data="users_back")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
