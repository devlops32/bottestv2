...
class Database:
    ...
    def add_user(self, user_id: int, username: str = None, first_name: str = None, last_name: str = None):
        """Добавляет пользователя с автоматической подпиской"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def get_all_users(self) -> List[int]:
        """Получает всех НЕЗАБЛОКИРОВАННЫХ пользователей (все подписаны автоматически)"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
    ...
    def toggle_subscription(self, user_id: int) -> bool:
        """Переключает подписку (НОВОЕ: всегда подписывает)"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
    ...
    def get_subscribed_users_count(self) -> int:
        """Все пользователи подписаны автоматически"""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
    ...
    # ============================================
    # ГЕНЕРАЦИЯ КОДОВ (НОВЫЙ ФОРМАТ: zipp + 6 цифр)
    # ============================================
    
    def generate_product_code(self) -> str:
        """Генерирует код в формате zipp + 6 цифр (например: zipp765432)"""
        while True:
