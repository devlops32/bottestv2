# handlers.py - Обработчики команд (ИСПРАВЛЕННЫЙ)
...
def get_all_users_for_mailing(target: str):
    ...
    if target == "all":
        ...
        return [u['user_id'] for u in active_users if u.get('subscribed', False)]
    elif target == "unsubscribed":
        return [u['user_id'] for u in active_users if not u.get('subscribed', False)]
    else:
        return []
...
async def unsubscribe_user(callback: CallbackQuery):
    ...
    db.toggle_subscription(user_id)
    await callback.message.edit_text(
        "❌ Вы отписались от уведомлений о новых товарах.\n"
        "Чтобы подписаться снова, напишите /subscribe"
    )
    await callback.answer()
...
🔴 Отписаны: {total_users - subscribed - blocked}
...
📬 Подписка: {'✅ Подписан' if user_data.get('subscribed', False) else '❌ Отписан'}
...
@router.callback_query(F.data.startswith("user_toggle_"))
async def user_toggle_subscription(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔️ Доступ запрещен", show_alert=True)
        return
    ...
    await callback.answer("✅ Подписка переключена!", show_alert=True)
...
async def user_block(callback: CallbackQuery):
    ...
    if user_data:
        ...
        await callback.message.edit_text(
            ...
            f"❌ Все команды будут игнорироваться\n\n"
...
async def mailing_select(callback: CallbackQuery, state: FSMContext):
    ...
    if target == "all":
        ...
        text = f"🟢 Подписанным ({total} чел.)"
    elif target == "unsubscribed":
        text = f"🔴 Отписавшимся ({total} чел.)"
    else:
...
async def auto_confirm_campaign(callback: CallbackQuery, state: FSMContext):
    ...
    # Импортируем auto_engine здесь, чтобы избежать циклического импорта
