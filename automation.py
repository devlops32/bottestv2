# automation.py - Модуль автоматизации (ИСПРАВЛЕН - БЕЗ ЦИКЛИЧЕСКИХ ИМПОРТОВ)
...
class AutoSellEngine:
    ...
    async def process_campaign_products(self, campaign: Dict):
        ...
        for product in products:
            ...
            if product_age >= 3 and product.in_stock:
                await self.simulate_activity(product_id)
    ...
    async def simulate_purchase(self, product: Product, campaign_id: int):
        ...
        products = db.get_all_products()
        product_data = next((p for p in products if p['name'] == product.name and p['quantity'] == product.quantity and p['city'] == product.city), None)
        product_code = product_data['product_code'] if product_data else 'AUTO'
...
def init_auto_engine(bot):
    """Инициализирует движок автоматизации"""
    global auto_engine
